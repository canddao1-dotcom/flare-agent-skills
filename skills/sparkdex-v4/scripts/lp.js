#!/usr/bin/env node
/**
 * SparkDex V4 (Algebra Integral) — LP Position Management
 * Flare Network
 * 
 * Usage:
 *   node lp.js positions --address 0x...
 *   node lp.js check     --address 0x...
 *   node lp.js mint      --token0 WFLR --token1 FXRP --amount0 1000 --amount1 100 --range 10
...
const NFPM_ABI = [
  ...
  'function positions(uint256 tokenId) external view returns (uint88 nonce, address operator, address token0, address token1, int24 tickSpacing, int24 tickLower, int24 tickUpper, uint128 liquidity, uint256 feeGrowthInside0LastX128, uint256 feeGrowthInside1LastX128, uint128 tokensOwed0, uint128 tokensOwed1)',
...
async function getSigner() {
  ...
  if (passwordPath && fs.existsSync(passwordPath)) {
    ...
    const candidates = ['~/.agent-keystore-password', path.join(path.dirname(keystorePath), '.password')];
...
async function fetchPositions(address, provider) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (poolAddr !== ethers.ZeroAddress) {
      ...
      [poolState, tickSpacing] = await Promise.all([pool.globalState(), pool.tickSpacing()]);
    }
    ...
    positions.push({
      ...
      currentFee: poolState ? Number(poolState.lastFee) : null,
...
function positionHealth(pos) {
  ...
  if (pctFromEdge < 0.10) {
    return { status: '🟡 NEAR EDGE', color: 'yellow' };
  }
...
async function cmdPositions(args) {
  ...
  if (positions.length === 0) {
    console.log(`\nNo SparkDex V4 positions found for ${address}\n`);
    return;
  }
  
  console.log(`\n SparkDex V4 Positions for ${address.slice(0, 8)}...${address.slice(-6)}`);
  ...
  for (const pos of positions) {
    ...
    const rangeStr = `${pLow.toPrecision(5)} - ${pHigh.toPrecision(5)}`;
    const feeStr = pos.currentFee !== null ? feeToPercent(pos.currentFee) : 'N/A';
...
async function cmdCheck(args) {
  ...
  let alerts = 0;
  console.log(`\n SparkDex V4 Health Check — ${positions.length} position(s)`);
  ...
  for (const pos of positions) {
    ...
    console.log(`  #${pos.tokenId}  ${sym0}/${sym1}  ${health.status}`);
    if (curPrice !== null) {
      console.log(`    Price: ${curPrice.toPrecision(5)}  Range: [${pLow.toPrecision(5)}, ${pHigh.toPrecision(5)}]`);
    }
    if (pos.currentFee !== null) {
      console.log(`    Dynamic fee: ${feeToPercent(pos.currentFee)}`);
    }
  }
...
async function cmdMint(args) {
  if (!args.token0 || !args.token1 || !args.amount0 || !args.amount1) {
    console.log('Usage: node lp.js mint --token0 WFLR --token1 FXRP --amount0 1000 --amount1 100 [--range 10] [--tickLower N --tickUpper N]');
    process.exit(1);
  }
  ...
  // Get pool state for tick spacing and current tick
  const factory = new ethers.Contract(CONTRACTS.Factory, FACTORY_ABI, provider);
  const poolAddr = await factory.poolByPair(token0.address, token1.address);
  if (poolAddr === ethers.ZeroAddress) throw new Error('Pool does not exist for this pair');
  
  const pool = new ethers.Contract(poolAddr, POOL_ABI, provider);
  const [state, tickSpacing] = await Promise.all([pool.globalState(), pool.tickSpacing()]);
  ...
  if (args.tickLower && args.tickUpper) {
    ...
    const rangePct = parseFloat(args.range || '10'); // default ±10%
  ...
  // Ensure ticks are divisible by spacing
  ...
  console.log(`\nMinting position: ${token0.symbol}/${token1.symbol}`);
  console.log(`  Current tick: ${currentTick}, range: [${tickLower}, ${tickUpper}]`);
  console.log(`  Tick spacing: ${spacing}, Fee: ${feeToPercent(state.lastFee)}`);
  ...
  for (const [token, amt] of [[token0, amount0], [token1, amount1]]) {
    ...
    if (allowance < amt) {
      ...
      const atx = await erc20.approve(nfpmAddr, ethers.MaxUint256);
      await atx.wait();
    }
  }
  ...
  const tx = await nfpm.mint({
    ...
    amount0Min: 0n, // Could add slippage protection
    amount1Min: 0n,
    recipient: myAddress,
    deadline,
  });
...
async function cmdAdd(args) {
  ...
  // Get position info to know tokens
...
async function cmdRemove(args) {
  ...
  console.log(`  Liquidity: ${pos.liquidity.toString()} → removing ${liquidityToRemove.toString()}`);
  ...
  // Burn if fully removed
  if (percent >= 100) {
    try {
      const tx3 = await nfpm.burn(BigInt(args.tokenId));
      await tx3.wait();
...
async function main() {
  ...
  try {
    switch (args._cmd) {
      ...
      default:
        console.log('SparkDex V4 LP Manager — Algebra Integral on Flare\n');
        console.log('Commands:');
        console.log('  positions --address 0x...');
        console.log('  check     --address 0x...');
        console.log('  mint      --token0 WFLR --token1 FXRP --amount0 1000 --amount1 100 [--range 10]');
